...
# Backup files
# duplicity --dry-run --verbosity 8 --allow-source-mismatch --full-if-older-than $FREQUENCY $FLAGS $SOURCE rclone://remote:$TARGET
duplicity --allow-source-mismatch --full-if-older-than $FREQUENCY $FLAGS $SOURCE rclone://remote:$TARGET >> /var/log/duplicity/$CODE.log 2>&1
...
duplicity remove-older-than 6M \
	--force \
	rclone://remote:$TARGET >> /var/log/duplicity/$CODE.log 2>&1
